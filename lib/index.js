...
// Normalizes away the domain (@s.whatsapp.net / @lid / @g.us) and any
// ":device" suffix before comparing, since the same person's JID can show up
// in different forms depending on chat context (this matters most in DMs,
// where there's no group participant list to cross-reference @lid vs phone
// number the way isOwnerOrSudo does for groups).
...
async function isSudo(userId) {
    try {
        ...
        return data.sudo.some(entry => normalizeJid(entry) === target);
    } catch (error) {
